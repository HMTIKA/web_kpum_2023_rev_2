<?php
$check_data_induk = query("SELECT * FROM data_induk_pemilih WHERE no_identitas = '$no_identitas'");
...
if(count($check_data_induk) > 0) {
    ...
    $check_data_registrasi = query("SELECT * FROM data_registrasi_pemilih WHERE no_identitas = '$no_identitas'");
    ...
    if(count($check_data_registrasi) > 0) {
        ...
        $voter_no_id = $check_data_induk[0]['no_identitas'];
        print_r($voter_no_id);
        $sql = "INSERT INTO data_registrasi_pemilih (id, no_identitas) VALUES ('', '$voter_no_id')";
